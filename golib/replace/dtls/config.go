...
type Config struct {
	...
	// RandomizationSeed, if set, is used for deterministic cipher suite and
	// extension randomization in flight handlers. This enables deeper
	// randomization (truncation) that hooks alone can't achieve, matching
	// Psiphon's approach of context-propagated seeds.
	RandomizationSeed []byte
